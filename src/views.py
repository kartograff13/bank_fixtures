...
def get_exchange_rates(currencies: list[str]) -> list[dict[str, Any]]:
    """Получение реальных курсов валют через Twelve Data API"""
    api_key = get_api_key()
    if not api_key:
        logger.warning("API ключ не найден. Возвращаем пустой список курсов валют.")
        return []
    ...
    for currency in currencies:
        ...
        try:
            ...
            if "rate" in data:
                ...
                logger.error(f"Ошибка получения курса {currency}: {data}")
                continue
...
def get_stock_prices(stocks: list[str]) -> list[dict[str, Any]]:
    """Получение реальных цен акций через Twelve Data API"""
    api_key = get_api_key()
    if not api_key:
        logger.warning("API ключ не найден. Возвращаем пустой список цен акций.")
        return []
    ...
    try:
        ...
        for stock in stocks:
            if stock in data and "price" in data[stock]:
                ...
                logger.error(f"Ошибка получения цены {stock}: {data.get(stock, 'Нет данных')}")
                continue
